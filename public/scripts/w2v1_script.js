...
canv.dataset.status.addEventListener( "change", () => {
    stat = canv.dataset.status;
         if( stat === "init" )   {   init_canv(); }
    else if( stat === "onehot" ) { onehot_canv(); }
} );
window.onload = function() { canv.dataset.status = "init"; canv.dataset.mode = "init"; }